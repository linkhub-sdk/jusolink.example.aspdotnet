<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="zipcode_search.aspx.cs" Inherits="Jusolink.Example.Example.zipcode_search" %>
	...
	<body class="body_width">
			<div id="content_result">
				...
				<fieldset class="fieldset5 left">
					<legend>주소검색</legend>
							<form method = "GET" id="zipcode_form" action="zipcode_search.aspx">
							    
							    <% if (result != null && result.searches != null) { %>
								    <input class= "txtZipcode left" type="text" id="IndexWord" name="IndexWord" value="<%= result.searches %>" tabindex=1 placeholder="예) 영동대로 517, 아셈타워, 삼성동 159"/>
    							<% } else { %>	    
    							    <input class= "txtZipcode left" type="text" id="Text1" name="IndexWord" value="" tabindex=1 placeholder="예) 영동대로 517, 아셈타워, 삼성동 159"/>
								<% } %>
			 ...
			 <div id="content_juso">
				<% if (result != null && result.juso != null) {
                        foreach (Jusolink.JusoInfo juso in result.juso) {
				%>
					...
					<fieldset class="fieldset6 left">
						...
						<p class="hand" onclick='detail("roadAddr", "<%=juso.zipcode%>", "<%= juso.sectionNum%>", "<%= juso.roadAddr1%>", "<%= juso.roadAddr2%>", "<%= juso.jibunAddr%>")'>
							<img class="imgAlign" src="../image/juso_icon_01.gif"/> <%= juso.roadAddr1%> <%= juso.roadAddr2%>
						</p>
						<p class="hand" onclick='detail("jibunAddr", "<%= juso.zipcode%>", "<%= juso.sectionNum%>", "<%= juso.roadAddr1%>", "<%= juso.roadAddr2%>", "<%= juso.jibunAddr %>")'>
							<img class="imgAlign" src="../image/juso_icon_02.gif"/> <%= juso.jibunAddr%>
						</p>

						<% if(juso.relatedJibun != null) {	%>
								<p> <img class="imgAlign" src="../image/juso_icon_03.gif"/> <%= juso.relatedJibun %></p>
						<%
							}
						%>			
					</fieldset>
					...
					<fieldset class="fieldset6 left">
						<div class ="select_box">
							...
							<div class="btn_list">
								...
								<a href="zipcode_search.aspx"><p class="btn_reset hand" tabindex=6>다시검색</p></a>
							</div>
						</div>
